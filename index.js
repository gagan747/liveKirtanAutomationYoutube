...
const recordStream = async (duty, endMilliseconds, to, from) => {
  ...
  const fileName = `${duty.trim()} Darbar Sahib ${
    getKirtanType(from, to) || "Kirtan Duty "
  }${datetime} - ${to})`;
    ...
    .on("error", (err) => {
      if (
        !err.message.includes(
          "ffmpeg exited with code 255: Exiting normally, received signal 15.",
        )
      )
...
setInterval(() => {
  //scheduled mp4 deleter if any file is left undeleted by any bug and also ragilistupdater is scheduled everyday at 1 am
  if (getIndianDate().getHours() === 1) {
    deleteMp4FilesIfAnyLeft();
    ragiListUpdateScheduler();
  } else if (getIndianDate().getHours() === 13) ragiListUpdateScheduler();
}, 3300000);
...
const getKirtanType = (from = "", to) => {
  ...
  else if (Number(to.split("-")[0]) === 8) kirtanType = "Bilawal Chowki ";
  return kirtanType;
};
