...
const convertRagiListToJson = async () => {
  ...
  return extractTables(getRagiListBuffer)
    .then((tables) => {
          ...
          return [
            {
              text: arr[0].text
                ...
                .map((data) =>
                  data
                    .split("Singh")
                    .map((data) => data.replace(/\s+/g, ""))
                    .join(" Singh "),
                )
                .join(" Bhai "), //clearing empty spaces in ragi name data as conversion has some garbage spaces so removing and purifying
            },
          ];
        return arr;
      });
      ...
      const timingsArrayGroupedByTable =
        generateKeertanTimingsArray(singleString);
      const duties = populateRagisToTimingsArray(
        ragisArray,
        timingsArrayGroupedByTable,
        singleString,
      );
      return new Promise((res, rej) => {
        fs.writeFile("ragiList.json", JSON.stringify(duties), (err, data) => {
          if (err) return rej("error in writing list");
...
const generateListValidity = (singleString) => {
  const matchedResult = singleString.match(
    /(\d{1} *?\d{1}-\d{1} *?\d{1}-\d{4,5}).*?to.*?((\d{1} *?\d{1}-\d{1} *?\d{1}-\d{4,5}))/im,
  );
  if (!matchedResult)
    throw new Error("Failed generating list validity of ragi list ");
  return {
    from: matchedResult[1].replace(/\s+/g, ""),
    to: matchedResult[2].replace(/\s+/g, ""),
  };
};
const generateKeertanTimingsArray = (str) => {
  ...
  const timingsArrayGroupedByTable = str
    ...
    .reduce(
      (p, c, index) => ({
        ...p,
        [`Group-${index + 1}`]: generateTimingsArrayFromString(c),
      }),
      {},
    ); //it generates all the timings in groups which are  generated by me in previous line
    ...
    if (arr[0].text.includes("Bhai")) {
      return [
        ...
        ...arr[0].text
          .split("Bhai")
          .filter(
            (data) => !(data == " " || data == "  ") && data.includes("Singh"),
          )
          .map((data) => "Bhai" + data),
      ];
    }
...
const populateRagisToTimingsArray = (
  ragisArray,
  timingsArrayGroupedByTable,
  singleString,
) => {
  ...
  for (let date = fromDate; date <= toDate; date.setDate(date.getDate() + 1)) {
    ...
    const group1Data = timingsArrayGroupedByTable["Group-1"].reduce(
      (p, c, index, arr) => {
        if (index === parseInt(arr.length / 2) + 1)
          //for repetition of same ragi to multiple times in table
          ragisArrayCurrentIndexForGroup1 =
            ragisArrayPopulationStartIndexForGroup1;
        if (index === parseInt(arr.length / 2) + 2)
          //condition for skipping multiple time entry of ragi in case of Asa Ki Vaar
          ragisArrayCurrentIndexForGroup1 += 1;
        let dutyObj = {
          from: c.split("to")[0],
          to: c.split("to")[1],
          duty: ragisArray[ragisArrayCurrentIndexForGroup1],
        };
        ragisArrayCurrentIndexForGroup1 += 1;
        return [...p, dutyObj];
      },
    ...
    const group2Data = timingsArrayGroupedByTable["Group-2"].reduce(
      (p, c, index, arr) => {
        if (index === parseInt(arr.length / 2))
          //for repetition of same ragi to multiple times in table
          ragisArrayCurrentIndexForGroup2 =
            ragisArrayPopulationStartIndexForGroup2;
        let dutyObj = {
          from: c.split("to")[0],
          to: c.split("to")[1],
          duty: ragisArray[ragisArrayCurrentIndexForGroup2],
        };
